...
# 扩展Keras BatchNormalization类，允许在需要时进行更改。
# 批处理小时批规格化对训练有负面影响，因此在小批处理时通常通过配置类设置冻结这个层
